            ...
            else
            {
                ...
                foreach ( $notification as $db_notification )
                {
                    ...
                    case  'Mcred_badge_earned'  :  $data['pm_notify'][$id] = $this->pm_generate_mycred_badge_earned_notice($db_notification,$id);
                                                    break;
                    case  'GroupEvent'          :  $data['pm_notify'][$id] = $this->pm_generate_group_event_notice($db_notification,$id);
                                                    break;
                    case  'UserStatus'          :  $data['pm_notify'][$id] = $this->pm_generate_user_status_notice($db_notification,$id);
                                                    break;
                    ...
                    foreach ( $notification as $db_notification )
                    {
                        ...
                        switch($type)
                        {
                            ...
                            case  'UserStatus' :  
                                ...
                                break;
                            default : $data['pm_notify'][$id] = __("no new notification","profilegrid-user-profiles-groups-and-communities");
                                break;
                        }
...
/*-----------------NOTIFICATION CREATION FUNCTIONS------------------*/  
            ...
            if($post->post_type=='pg_groupwalls')
            {
                ...
                if(!empty($unique_ids)  &&  empty($is_added))
                {
                    ...
                    {  
                        ...
                        {
                            $data = array('type'=>'WallPost','sid'=>$author,'rid'=>$uid,'timestamp'=>$timestamp,'description'=>$title,'status'=>1,'meta'=>$meta);
                            $arg = array('%s','%s','%s','%s','%s','%s','%s');
                            $gid = $dbhandler->insert_row('NOTIFICATION', $data,$arg);
                        ...
                        if($uid==$author && $post_status=='publish')
                        {
                            $data = array('type'=>'WallPostOwner','sid'=>$author,'rid'=>$uid,'timestamp'=>$timestamp,'description'=>$title,'status'=>1,'meta'=>$meta);
                            $arg = array('%s','%s','%s','%s','%s','%s','%s');
                            $gid = $dbhandler->insert_row('NOTIFICATION', $data,$arg);
                            add_post_meta($ID,'pg_notification_added','1');
        ...
        public function pg_new_group_event_notification($ID, $gids)
        {
            ...
            if(!empty($userids))
            {
                ...
                {  
                    $data = array('type'=>'GroupEvent','sid'=>$author,'rid'=>$uid,'timestamp'=>$timestamp,'description'=>$title,'status'=>1,'meta'=>$meta);
                    $arg = array('%s','%s','%s','%s','%s','%s','%s');
                    $gid = $dbhandler->insert_row('NOTIFICATION', $data,$arg);
                    add_post_meta($ID,'pg_notification_added','1');
        ...
        public function pg_new_user_status_notification($status_id,$data)
        {
            ...
            if(!empty($users))
            {
                foreach($users as $user)
                {  
                    $data1 = array('type'=>'UserStatus','sid'=>$sid,'rid'=>$user->ID,'timestamp'=>$timestamp,'description'=>$description,'status'=>1,'meta'=>$meta);
                    $arg = array('%s','%s','%s','%s','%s','%s','%s');
        ...
        public function pm_added_new_message_notification($rid,$sid,$message){
         ...
         public function pm_added_earned_new_badge_notification($uid,$new_level,$badge){
            ...
            //$rid = get_post_field( 'post_author',$comment->comment_post_ID);
            $timestamp = current_time('mysql',true);
            $meta = array('new_level'=>$new_level,'badge'=>$badge);
            //add something to meta if you want
            $message = '';
            $meta= maybe_serialize($meta);
...
//            $current_user_group = $pmrequests->profile_magic_get_user_field_value($current_uid,'pm_group');            
           ...
           // if((in_array($current_user_group, $receivers['group']) || in_array($current_uid, $receivers['uid']) ) && !in_array($current_uid, $receivers['exclude']))
            ...
            $sender_avatar = get_avatar($notif_sender_id, 50, '', false, array('class' => 'pm-user-profile','force_display'=>true));
...
//            $current_user_group = $pmrequests->profile_magic_get_user_field_value($current_uid,'pm_group');            
  ...
  //          $this->pm_change_notification_status($id,2);
       ...
       <path d="M0 0h24v24H0z" fill="none"/>
                ...
                <div class="pm-notification-title pm-pad10 ">'.$bold.__('New Profile Status Added','profilegrid-user-profiles-groups-and-communities').$bold_close.'</div>
                <div class="pm-notification-description-wrap pm-dbfl pm-pad10  ">
                    <div class="pm-notification-profile-image pm-difl">'.$sender_avatar.'</div>
                    <div class="pm-notification-description pm-difl">
                        <div class="pm-notification-user pm-color"><a href="'.$sender_profile_url.'" target="_blank">'.$sender_name.'</a></div>
                        <div class="pm-notification-user-activity">'.$description.'</div>
                    </div>
                </div>
                <div class="pm-notification-footer pm-dbfl"><div class="pm-notification-buttons"><a target="_blank" href="'.$sender_profile_url.'">'.__('View','profilegrid-user-profiles-groups-and-communities').'</a></div></div>
            </div>
        </div>';
